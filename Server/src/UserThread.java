			...
			while (true) {
				...
				// 방 참여
					...
					if(str.equals("READY_FOR_JOIN")) { // 후 처리 전 메세지 받기
						...
						// 참여한 방에 참여자 정보 전달
						ArrayList<User> joinUsers = roomInstance.getRoomInfo(Integer.parseInt(joinRoomStr[2])).entry; // 참여한 방의 유저 리스트
				...
				// 방 나가기
					...
					roomInstance.getRoomInfo(Integer.parseInt(outRoomStr[2])).RemoveEntry(userInfo); // 해당 방의 유저 목록에서 나간 유저 삭제
					// 나간 유저 정보 전달
					ArrayList<User> joinUsers = roomInstance.getRoomInfo(Integer.parseInt(outRoomStr[2])).entry; // 참여한 방의 유저 리스트
					if (joinUsers.size() != 0) {
